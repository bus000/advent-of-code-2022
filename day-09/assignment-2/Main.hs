{- A rope snaps! Suddenly, the river is getting a lot closer than you remember.
 - The bridge is still there, but some of the ropes that broke are now whipping
 - toward you as you fall through the air!
 -
 - The ropes are moving too quickly to grab; you only have a few seconds to
 - choose how to arch your body to avoid being hit. Fortunately, your simulation
 - can be extended to support longer ropes.
 -
 - Rather than two knots, you now must simulate a rope consisting of ten knots.
 - One knot is still the head of the rope and moves according to the series of
 - motions. Each knot further down the rope follows the knot in front of it
 - using the same rules as before.
 -
 - Using the same series of motions as the above example, but with the knots
 - marked H, 1, 2, ..., 9, the motions now occur as follows:
 -
 -     == Initial State ==
 -
 -     ......
 -     ......
 -     ......
 -     ......
 -     H.....  (H covers 1, 2, 3, 4, 5, 6, 7, 8, 9, s)
 -
 -     == R 4 ==
 -
 -     ......
 -     ......
 -     ......
 -     ......
 -     1H....  (1 covers 2, 3, 4, 5, 6, 7, 8, 9, s)
 -
 -     ......
 -     ......
 -     ......
 -     ......
 -     21H...  (2 covers 3, 4, 5, 6, 7, 8, 9, s)
 -
 -     ......
 -     ......
 -     ......
 -     ......
 -     321H..  (3 covers 4, 5, 6, 7, 8, 9, s)
 -
 -     ......
 -     ......
 -     ......
 -     ......
 -     4321H.  (4 covers 5, 6, 7, 8, 9, s)
 -
 -     == U 4 ==
 -
 -     ......
 -     ......
 -     ......
 -     ....H.
 -     4321..  (4 covers 5, 6, 7, 8, 9, s)
 -
 -     ......
 -     ......
 -     ....H.
 -     .4321.
 -     5.....  (5 covers 6, 7, 8, 9, s)
 -
 -     ......
 -     ....H.
 -     ....1.
 -     .432..
 -     5.....  (5 covers 6, 7, 8, 9, s)
 -
 -     ....H.
 -     ....1.
 -     ..432.
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     == L 3 ==
 -
 -     ...H..
 -     ....1.
 -     ..432.
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     ..H1..
 -     ...2..
 -     ..43..
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     .H1...
 -     ...2..
 -     ..43..
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     == D 1 ==
 -
 -     ..1...
 -     .H.2..
 -     ..43..
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     == R 4 ==
 -
 -     ..1...
 -     ..H2..
 -     ..43..
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     ..1...
 -     ...H..  (H covers 2)
 -     ..43..
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     ......
 -     ...1H.  (1 covers 2)
 -     ..43..
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     ......
 -     ...21H
 -     ..43..
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     == D 1 ==
 -
 -     ......
 -     ...21.
 -     ..43.H
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     == L 5 ==
 -
 -     ......
 -     ...21.
 -     ..43H.
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     ......
 -     ...21.
 -     ..4H..  (H covers 3)
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     ......
 -     ...2..
 -     ..H1..  (H covers 4; 1 covers 3)
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     ......
 -     ...2..
 -     .H13..  (1 covers 4)
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     ......
 -     ......
 -     H123..  (2 covers 4)
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     == R 2 ==
 -
 -     ......
 -     ......
 -     .H23..  (H covers 1; 2 covers 4)
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 -     ......
 -     ......
 -     .1H3..  (H covers 2, 4)
 -     .5....
 -     6.....  (6 covers 7, 8, 9, s)
 -
 - Now, you need to keep track of the positions the new tail, 9, visits. In this
 - example, the tail never moves, and so it only visits 1 position. However, be
 - careful: more types of motion are possible than before, so you might want to
 - visually compare your simulated rope to the one above.
 -
 - Here's a larger example:
 -
 -     R 5
 -     U 8
 -     L 8
 -     D 3
 -     R 17
 -     D 10
 -     L 25
 -     U 20
 -
 - These motions occur as follows (individual steps are not shown):
 -
 -     == Initial State ==
 -
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ...........H..............  (H covers 1, 2, 3, 4, 5, 6, 7, 8, 9, s)
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -
 -     == R 5 ==
 -
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ...........54321H.........  (5 covers 6, 7, 8, 9, s)
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -
 -     == U 8 ==
 -
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ................H.........
 -     ................1.........
 -     ................2.........
 -     ................3.........
 -     ...............54.........
 -     ..............6...........
 -     .............7............
 -     ............8.............
 -     ...........9..............  (9 covers s)
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -
 -     == L 8 ==
 -
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ........H1234.............
 -     ............5.............
 -     ............6.............
 -     ............7.............
 -     ............8.............
 -     ............9.............
 -     ..........................
 -     ..........................
 -     ...........s..............
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -
 -     == D 3 ==
 -
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     .........2345.............
 -     ........1...6.............
 -     ........H...7.............
 -     ............8.............
 -     ............9.............
 -     ..........................
 -     ..........................
 -     ...........s..............
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -
 -     == R 17 ==
 -
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ................987654321H
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ...........s..............
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -
 -     == D 10 ==
 -
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ...........s.........98765
 -     .........................4
 -     .........................3
 -     .........................2
 -     .........................1
 -     .........................H
 -
 -     == L 25 ==
 -
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ...........s..............
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     H123456789................
 -
 -     == U 20 ==
 -
 -     H.........................
 -     1.........................
 -     2.........................
 -     3.........................
 -     4.........................
 -     5.........................
 -     6.........................
 -     7.........................
 -     8.........................
 -     9.........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ...........s..............
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -
 -     Now, the tail (9) visits 36 positions (including s) at least once:
 -
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     ..........................
 -     #.........................
 -     #.............###.........
 -     #............#...#........
 -     .#..........#.....#.......
 -     ..#..........#.....#......
 -     ...#........#.......#.....
 -     ....#......s.........#....
 -     .....#..............#.....
 -     ......#............#......
 -     .......#..........#.......
 -     ........#........#........
 -     .........########.........
 -
 - Simulate your complete series of motions on a larger rope with ten knots. How
 - many positions does the tail of the rope visit at least once?
 -}
module Main where

import AdventOfCode
import qualified Data.Text as T
import qualified Data.List as L
import qualified Data.Set as Set
import Text.Parsec ((<|>))
import qualified Text.Parsec as P
import qualified Text.Parsec.Number as P
import qualified Control.Monad.State as M

main :: IO ()
main = defaultMain parseInput handleInput

data Position = Position
    { _x :: !Int
    , _y :: !Int
    } deriving (Show, Eq, Ord)

data Rope = Rope
    { _head       :: !Position
    , _tailDeltas :: ![Position]
    } deriving (Show, Eq, Ord)

handleInput :: [Position] -> IO ()
handleInput = print . Set.size . Set.fromList . map ropeTail . simulateRope 9

simulateRope :: Word -> [Position] -> [Rope]
simulateRope tailLength = L.scanl' move initialRope
  where
    intTailLength = fromIntegral tailLength
    initialRope = Rope origin (replicate intTailLength origin)
    origin = Position 0 0

ropeHead :: Rope -> Position
ropeHead = _head

ropeTail :: Rope -> Position
ropeTail (Rope (Position x y) rTail) = Position (x + dx) (y + dy)
  where
    dx = sum . map _x $ rTail
    dy = sum . map _y $ rTail

move :: Rope -> Position -> Rope
move (Rope rHead rTail) headMovement = Rope rHead' rTail'
  where
    rHead' = rHead `plus` headMovement
    rTail' = M.evalState (mapM moveTail rTail) headMovement

    moveTail :: Position -> M.State Position Position
    moveTail previousDelta = do
        previousMove <- M.get
        let delta = previousDelta `plus` invert previousMove
            adjustment = tailAdjustment delta
            newDelta = delta `plus` adjustment
        M.put adjustment
        return newDelta

    tailAdjustment (Position dx dy)
        | dx > 1 = Position (-1) (toward0 dy)
        | dx < -1 = Position 1 (toward0 dy)
        | dy > 1 = Position (toward0 dx) (-1)
        | dy < -1 = Position (toward0 dx) 1
        | otherwise = Position 0 0

    toward0 x
        | x < 0 = 1
        | x > 0 = -1
        | otherwise = 0

plus :: Position -> Position -> Position
plus (Position x1 y1) (Position x2 y2) = Position (x1 + x2) (y1 + y2)

invert :: Position -> Position
invert (Position x y) = Position (-x) (-y)

parseInput :: T.Text -> Either P.ParseError [Position]
parseInput = P.parse (parsePositions <* P.eof) ""

parsePositions :: P.Parsec T.Text () [Position]
parsePositions = concat <$> doParsePositions `P.endBy` P.newline
  where
    doParsePositions = do
        position <- parsePosition
        _ <- P.char ' '
        count <- P.int

        return $ replicate count position

parsePosition :: P.Parsec T.Text () Position
parsePosition = north <|> east <|> south <|> west
  where
    north = P.char 'U' *> pure (Position 0 1)
    east = P.char 'R' *> pure (Position 1 0)
    south = P.char 'D' *> pure (Position 0 (-1))
    west = P.char 'L' *> pure (Position (-1) 0)
