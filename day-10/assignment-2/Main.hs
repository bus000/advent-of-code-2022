...
handleInput = T.putStrLn . drawScreen . runProgram . expandProgram

drawScreen :: [CPU] -> T.Text
drawScreen [] = ""
drawScreen (_:cpus) =
    T.intercalate "\n" . map T.pack . chunksOf width . map drawpixel $ cpus
  where
    drawpixel cpu = if isOn cpu then '#' else ' '
...
isOn :: CPU -> Bool
isOn (CPU register _ cycles) = diff < 2
  where
    horizontalPosition = cycles `mod` 40
    diff = abs $ register - horizontalPosition

runProgram :: Program -> [CPU]
runProgram = L.scanl' runInstruction initialCpu
  where
    initialCpu = CPU 1 0 (-1)
