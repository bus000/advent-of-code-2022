...
data Rucksack a = Rucksack
    { _left  :: !(Compartment a)
    , _right :: !(Compartment a)
    } deriving (Show, Eq, Ord)

type Compartment a = Set.Set a

handleInput :: [Rucksack Char] -> IO ()
handleInput = print . sum . map prioritizeSack

prioritizeSack :: Rucksack Char -> Word
prioritizeSack (Rucksack left right)
    = sum
    . map priority
    . Set.toList
...
fromList :: Ord a => [a] -> Rucksack a
fromList content = Rucksack left right
  where
    halfLength = length content `div` 2
    left = Set.fromList $ take halfLength content
    right = Set.fromList $ drop halfLength content

parseInput :: T.Text -> Either P.ParseError [Rucksack Char]
parseInput = P.parse (parseRucksacks <* P.eof) ""

parseRucksacks :: P.Parsec T.Text () [Rucksack Char]
parseRucksacks = parseRucksack `P.endBy` P.newline

parseRucksack :: P.Parsec T.Text () (Rucksack Char)
parseRucksack = fromList <$> P.many (P.oneOf $ ['a'..'z'] ++ ['A'..'Z'])
